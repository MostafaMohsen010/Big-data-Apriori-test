	...
	for r in rules:
	    ...
	    for c,l in enumerate(r.rhs):
	        ...
	        if(c == len(r.rhs)-1):
	            break
	        print(" , ",end="")
	    print(") Confidence=",r.confidence,"  Lift=",r.lift,"\n\n")
  ...
  def visualize(self): 
    ...
    for c,l in enumerate(self.rhs):
      ...
      if(c == len(self.rhs)-1):
        break
      print(" & ",end="")
    print(") [ Confidence=",self.confid," , Lift=",self.sup_lhsandrhs/(self.sup_lhs*self.sup_rhs)," , Leverage=",self.sup_lhsandrhs-(self.sup_lhs*self.sup_rhs),"] \n\n")
